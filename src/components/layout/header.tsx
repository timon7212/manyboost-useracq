...
export function Header() {
  ...
  const navItems = [
    ...
    // { href: "/creators", label: "Creators", description: "Earn Money" }, // Temporarily disabled
      ...
      >
        ...
        <div className="relative max-w-[1200px] mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link href="/" className="group relative">
            <motion.span 
              className="text-[22px] font-semibold tracking-[-0.02em] relative z-10"
              style={{
          ...
          <nav className="hidden md:flex items-center absolute left-1/2 -translate-x-1/2">
            <div className="flex items-center bg-[#0a0a0a]/80 rounded-full p-1.5 border border-[#222] shadow-lg shadow-black/20">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative px-6 py-2.5 rounded-full text-[13px] font-medium transition-all duration-300 ${
                    isActive(item.href)
                      ? "text-white"
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:flex items-center gap-1">
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
              >
                <svg width="17" height="17" viewBox="0 0 24 24" fill="#555" className="group-hover:fill-[#e97714] transition-colors duration-300">
            ...
            {/* Divider */}
            <div className="hidden lg:block w-px h-5 bg-gradient-to-b from-transparent via-[#333] to-transparent" />

            {/* CTA Button */}
            <motion.button
              onClick={() => setLeadModalOpen(true)}
              className="px-6 py-2.5 bg-[#e97714] hover:bg-[#f08524] text-black font-semibold rounded-full text-[13px] transition-all duration-300 shadow-lg shadow-[#e97714]/25 hover:shadow-[#e97714]/40"
